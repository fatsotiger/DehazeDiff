...
def SaveAimg(data, name, address, types=True):
    """

    :param data: [4,16,128,128]
    :param name: 保存名称
    :param address: 保存地址
    :param types: Ture：每个各维度保存  Fals：热力图
    """
    data = data[0].squeeze(0).permute(1, 2, 0).cpu().detach().numpy()
    if types:
        n = data.shape[2]
        for i in range(n):
            if i+1 > 16:
                break
            plt.subplot(4, 4, i + 1)
            plt.imshow(data[:, :, i], cmap='jet', interpolation='nearest')

            plt.xticks([])
            plt.yticks([])
        plt.subplots_adjust(wspace=0.0, hspace=0.04, left=0.2, right=0.79)
    else:
        plt.subplot(1, 1, 1)
        ...
        plt.yticks([])
    t=0
    plt.savefig(address + name +str(t)+ '.png')
    t += 1
    plt.clf()
def SaveFImg(data,name):
    print('Save_F_IMG')
    """

    :param data: data = [a = torch.rand(4,16,128,128),b = torch.rand(4,16,128,128)]
    :param name: name = ['潜在','信息素']
    """
    x = 0
...
class SDE(abc.ABC):
    def __init__(self, T=20, device=None):
        self.T = T
        self.dt = 1 / T
        self.device = device
...
class IRSDE(SDE):
    ...
    def score_fn(self, x, t, **kwargs):
        ...
        import torch
        # print('pt  ____________________')
        # print(x.shape)
        # from thop import profile
        # flops, params = profile(self.model, inputs=(x, self.mu, t))
    ...
    def noise_fn(self, x, t, **kwargs):
        ...
        # print(t)
        """
        FLOPs = 32.365921024G+4.5G
        Params = 76.563792M+29M
    ...
    def reverse_sde(self, xt, T=-1, save_states=True, save_dir='D:\\Diffusion_way\\image-restoration-sde-main\\image-restoration-sde-main\\codes\\config\\latent-dehazing\\sde_state', **kwargs):
        # print('reverse_sde')
        # print('T',self.T)
        T = self.T if T < 0 else T
        x = xt.clone()
        y = x
        sign = 3
           # alpha_schedule[t] / (alpha_schedule[t] + alpha_schedule[t - S])
        idx = T
        for t in tqdm(reversed(range(1, T + 1))):
            # score = self.score_fn(x, t, **kwargs)
            # x = self.reverse_sde_step(x, score, t)
            # tvutils.save_image(x.transpose(0, 1)[3].data, f'output/state/state_orign{idx}.png', normalize=False)
            # idx -= 1

            if t%sign==0 and t + sign <= T   :
                score = self.score_fn(x, t, **kwargs)
                x = self.reverse_sde_step(x, score, t)
                ...
                for i in reversed(range(t, t + sign)):
                    ...
                    score = self.score_fn(x, i, **kwargs)
                    x = self.reverse_sde_step(x, score, i)
                    # tvutils.save_image(x.transpose(0, 1)[3].data, f'output/state/state_Back_{idx_}.png', normalize=False)
                    idx_ -= 1
                y = x
                ...
                # tvutils.save_image(x.transpose(0, 1)[3].data, f'output/state/state_{idx}.png', normalize=False)
            idx-=1
            # if True: # only consider to save 100 images
            #     # print('save,T = ',t)
            #
            #     try:
            #         interval = self.T // 500
            ...
            #             os.makedirs(save_dir, exist_ok=True)
            #             # print()
            #             tvutils.save_image(x.transpose(0, 1).data, f'{save_dir}/state_{idx}.png', normalize=False)
            #             # x_np = x.squeeze(0).data.cpu().numpy()  # 去掉批次维度，得到 [8, 128, 128]
            #             #
            #             # # 对通道维度进行聚合（例如取平均值）
            #             # x_aggregated = x_np[3]*255   # np.mean(x_np, axis=0)*255  # 对 8 个通道取平均值，得到 [128, 128]
            #             # x_aggregated = (x_aggregated - np.min(x_aggregated)) / (
            #             #             np.max(x_aggregated) - np.min(x_aggregated))
            #             #
            #             # # 创建热力图
            #             # plt.figure(figsize=(10, 10))
            #             # plt.imshow(x_aggregated, cmap='hot', interpolation='nearest')
            #             # # plt.colorbar()  # 添加颜色条
            #             # plt.axis('off')
            #             #
            #             # # 保存热力图
            #             # plt.savefig(f'{save_dir}/state_{idx}_heatmap.png', bbox_inches='tight', pad_inches=0.1)
    ...
    def generate_random_states(self, x0, mu): # 清晰 有雾 特征图谱
        x0 = x0.to(self.device)
        mu = mu.to(self.device)
        # print(x0.shape) # torch.Size([4, 8, 64, 64])

        self.set_mu(mu)

        batch = x0.shape[0] # 批次大小

        timesteps = torch.randint(1, self.T + 1, (batch, 1, 1, 1)).long() # time
        # print('time = ',timesteps) # time =  tensor([[[[3]]]]) # time =  tensor([[[[5]]],
        #
        #
        #         [[[3]]]])
        """
            state_mean =  torch.Size([4, 8, 64, 64])
            snoise_level=  torch.Size([4, 1, 1, 1])
        """
        ...
        # print('snoise_level= ', noise_level.shape)
        noisy_states = noises * noise_level + state_mean # 噪声 + 降质后的特征群

        # SaveFImg([state_mean,noisy_states],['降质-','加入噪声-'])



        # print()
        # tvutils.save_image(noisy_states.transpose(0, 1).data, 'x.png', normalize=False)

        return timesteps, noisy_states.to(torch.float32)



    def print_img(self,data):
        data = data[0].squeeze(0).permute(1, 2, 0).cpu().detach().numpy()
        n = data.shape[2]

    def noise_state(self, tensor):
        state_mean = torch.randn_like(tensor)
        x = tensor + state_mean * self.max_sigma
        # SaveFImg([tensor, x], ['降质', '加入噪声'])
        # print('begin')
        # print_img(x)
        # print_img(tensor)
        return x
